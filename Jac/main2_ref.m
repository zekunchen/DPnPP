...
model.modelPath('C:\Users\zekun\Desktop\EIT_MATLAB\1_Array\ref');
...
model.component('comp1').geom('geom1').feature('imp1').set('filename', 'C:\Users\zekun\Desktop\EIT_MATLAB\1_Array\EMP16.mphbin');
...
%     for j=(i+2):T %104 使用
    for j=1:16
        I(cIndex,1)=mpheval(model,{'V'},'edim',1,'selection',indexMap(j));%函数在说明手册258页
        J(cIndex,1)=mpheval(model,{'V'},'edim',1,'selection',indexMap(j+1)); % Meas
        cIndex=cIndex+1;
    end
...
V=abs(aa-bb);
eval(['save ./ref/v' num2str(n) '.txt V -ASCII'])
% disp('Measurement Voltage Caculation Done!');
toc

% figure;
% plot(V,'o-');
% axis tight
% title('Simulated voltage data'); 
